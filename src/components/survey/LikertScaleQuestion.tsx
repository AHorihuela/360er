...
export function LikertScaleQuestion({
  ...
  useEffect(() => {
    ...
    if (!options || options.length === 0) {
      console.error('LikertScaleQuestion: No options provided for question', id);
    } else if (!Array.isArray(options)) {
      console.error('LikertScaleQuestion: Options is not an array for question', id, options);
    }
  }, [id, options]);

  // Add fallback for missing options
  if (!options || !Array.isArray(options) || options.length === 0) {
    console.warn('Using fallback options for Likert scale question:', id);
    ...
    return (
      <div className="space-y-3">
        <div className="space-y-1">
          ...
          <div className="text-sm text-yellow-500">Warning: Using default options. Please check console.</div>
        </div>
